...
def compute_pna_degrees(
    ...
    for data in tqdm(train_loader):
        ...
        try:
            ...
            logger.error(f"clique edge index {data.clique_edge_index[1]}")
            logger.error(f"clique x {data.clique_x}")
            logger.error(f"clique shape {data.clique_x.shape}")
            logger.error(f"atom shape {data.mol_x.shape[0]}")
            break
        ...
        if save_interpret:
            for pair_id in df[matching_row]["ID"]:
                ...
                if not successful_ligand:
                    logger.warning(
                        f"Ligand Intepretation for {pair_id} failed due to not matching atom order."
                    )
                ## STORE Fingerprint
                np.save(
