...
class Trainer(object):
    def __init__(
        ...
        if finetune_modules is not None:
            self.optimizer = self.model.freeze_backbone_optimizers(
                finetune_modules,
                weight_decay=wdecay,
                learning_rate=lrate,
                betas=betas,
            ...
            logger.info(
                "freezing backbone and now training only the finetune modules..."
            )
        ...
        with output(initial_len=11, interval=0) as output_lines:
            for epoch in range(1, self.num_epochs + 1):
                ...
                for data in train_loader:
                    ...
                    reg_pred, cls_pred, mcls_pred, sp_loss, o_loss, cl_loss, _ = (
                        self.model(
                            # Molecule
                            mol_x=data.mol_x,
                    ...
                    if reg_pred is not None:
                        ...
                        reg_loss = (
                            self.regression_loss(reg_pred, reg_y)
                    ...
                    if cls_pred is not None:
                        ...
                        cls_loss = (
                            self.classification_loss(cls_pred, cls_y)
                    ...
                    if mcls_pred is not None:
                        mcls_y = data.mcls_y
                        mcls_loss = (
                            self.mclassification_loss(mcls_pred, mcls_y)
                            * self.multiclassification_weight
                        )
                        loss_val += mcls_loss
                ...
                # Initialize test_result and val_result to avoid UnboundLocalError
                test_result = {}
                val_result = {}
                ...
                if epoch % evaluate_epoch == 0 and val_loader is not None:
                    ...
                    if better_than_previous:
                        ...
                        if self.skip_test_during_train is False:
                            ...
                            test_result = {
                                k: round(v, 4) for k, v in test_result.items()
                            }
                        else:
                            test_result = {}
                ...
                output_lines[2] = "-" * 40
                output_lines[3] = (
                    f"Epoch {epoch:03d} with LR {curr_lr_rate:.6f}: Model Results"
                )
                output_lines[4] = "-" * 40
        ...
        with output(initial_len=11, interval=0) as output_lines:
            while iter_num < self.total_iters:
                for data in train_loader:
                    ...
                    if (iter_num % evaluate_step == 0) or (
                        iter_num == self.total_iters
                    ):
                        ...
                        output_lines[2] = "-" * 40
                        output_lines[3] = (
                            f"Training Step: {iter_num} with LR {curr_lr_rate:.6f}: Model Results"
                        )
                        output_lines[4] = "-" * 40
                        output_lines[5] = train_str1
                        output_lines[6] = train_str2
        ...
        with torch.no_grad():
            for data in tqdm(data_loader, leave=False, desc="evaluating"):
                ...
                if reg_pred is not None:
                    ...
                    reg_loss = (
                        self.regression_loss(reg_pred, reg_y) * self.regression_weight
                    )
                    loss_val += reg_loss
                    reg_loss = reg_loss.item()
    ...
    def pred(self, model: nn.Module, data_loader: DataLoader, store_interpret=True):
        ...
        with torch.no_grad():
            for data in tqdm(data_loader):
                ...
                if store_interpret:
                    attention_dict = store_attention_result(
                        attention_dict, interaction_key, reg_tuples, cls_tuples
                    )
    ...
    def get_lr(self, iter):
        ...
        decay_ratio = (iter - self.warmup_iters) / (
